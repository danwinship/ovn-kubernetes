...
var _ = ginkgo.FDescribe("EgressService", func() {
	const (
	...
	ginkgo.BeforeEach(func() {
		...
		externalContainer = provider.ExternalContainer{Name: externalContainerName, Image: images.AgnHost(),
			Network: network,
			CMD:     []string{"netexec", fmt.Sprintf("--http-port=%d", externalContainerPort)}}
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerPort)
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerPort); err != nil {
						return err
					}
				}
				return nil
...
`)
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerPort)
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, dstIP, externalContainerPort)
...
`)
	...
	ginkgo.FDescribeTable("Should validate the egress SVC SNAT functionality against host-networked pods",
		func(protocol v1.IPFamily) {
			skipIfProtoNotAvailableFn(protocol, externalContainer)
			ginkgo.By("Creating the backend pods")
			podsCreateSync := errgroup.Group{}
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainerPort)
				}, 5*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with loadbalancer's ingress ip")
			}

			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainerPort); err != nil {
						return err
					}
				}
				return nil
...
`)
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, externalContainerIP, externalContainerPort)
				}, 3*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with node's ip")
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, egressIP.String(), dstIP, externalContainerPort)
...
`)
	...
	ginkgo.Describe("[multi-net] Multiple Networks, external clients sharing ip", func() {
		/*
...
`)
				...
				for _, net := range []*netSettings{net1, net2} {
					for i, pod := range net.createdPods {
						...
						gomega.Eventually(func() error {
							return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, dst, externalContainerPort)
						...
						gomega.Consistently(func() error {
							return curlAgnHostHostnameFromPod(f.Namespace.Name, pod, net.containerName, dst, externalContainerPort)
				...
				gomega.Consistently(func() error {
					for _, pod := range append(net1.createdPods, net2.createdPods...) {
						err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, "", dst, externalContainerPort)
...
func curlAgnHostClientIPFromPod(namespace, pod, expectedIP, dstIP string, containerPort int32) error {
...
func curlAgnHostHostnameFromPod(namespace, pod, expectedHostname, dstIP string, containerPort int32) error {
	dst := net.JoinHostPort(dstIP, fmt.Sprint(containerPort))
...
func reachAllServiceBackendsFromExternalContainer(container provider.ExternalContainer, svcIP string, svcPort int32, svcPods []string) {
	...
	for i := 0; i < 10*len(svcPods); i++ {
		out, err := provider.Get().ExecExternalContainerCommand(container, []string{"curl", "-s", fmt.Sprintf("http://%s/hostname", dst)})
...
func setBlackholeRoutingTableOnNodes(infra provider.Context, nodes []v1.Node, extContainer provider.ExternalContainer, routingTable string, useV4 bool) {
	for _, node := range nodes {
		if useV4 {
			setBlackholeRoutesOnRoutingTable(infra, node.Name, extContainer.GetIPv4(), routingTable)
			continue
		}
		if extContainer.IsIPv6() {
			setBlackholeRoutesOnRoutingTable(infra, node.Name, extContainer.GetIPv6(), routingTable)
		}
	}
}
